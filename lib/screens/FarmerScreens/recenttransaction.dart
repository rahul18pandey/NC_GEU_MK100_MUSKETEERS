...
class _RecentTransactionState extends State<RecentTransaction> {
  ...
  var items = ['1', '2', '3', '4','5'];
  String a,token;
  QuerySnapshot collabdata,buyersdata,servicedata;
  ...
  Widget build(BuildContext context) {
    if(transactiondata!=null && farmerdata!=null &&collabdata!=null&&buyersdata!=null &&servicedata!=null){
      for(int i=0;i<transactiondata.documents.length;i++){
        if(transactiondata.documents[i].data['Farmer_email']==fobj.user.email){
          count++;
          break;
        }
      }
      if(count!=0) {
        return new Scaffold(
            ...
            appBar: AppBar(
              centerTitle:true,
              title: Text("My Transactions", textAlign: TextAlign.center,style: TextStyle(fontFamily: 'Montserrat',fontWeight: FontWeight.w600),),
  ...
  Widget buildTransaction(BuildContext context, int i) {
    ...
    for(int k=0;k<buyersdata.documents.length;k++){
      if(buyersdata.documents[k].documentID==transactiondata.documents[i].data['Email']){
    ...
    if(transactiondata.documents[i].data['Type']=='transport' ||
        transactiondata.documents[i].data['Type']=='irrigation'||
        transactiondata.documents[i].data['Type']=='coldstorage/warehouse'||
        transactiondata.documents[i].data['Type']=='packaging'||
        transactiondata.documents[i].data['Type']=='fertilizer/pesticides'
    ){
      isfarmer=true;
    }
    for (int l = 0; l < servicedata.documents.length; l++) {
      if (servicedata.documents[l].documentID ==
          transactiondata.documents[i].data['Email']) {
        name = servicedata.documents[l].data['Name'];
        break;
      }
    }
    for (int l = 0; l < buyersdata.documents.length; l++) {
      if (buyersdata.documents[l].documentID ==
          transactiondata.documents[i].data['Email']) {
    ...
    if(!isfarmer) {
      for (q = 0; q < collabdata.documents.length; q++) {
        if (collabdata.documents[q].data['Farmeremail'] == fobj.user.email &&
            collabdata.documents[q].data['Buyeremail'] ==
                transactiondata.documents[i].data['Email']) {
          isrequest = true;
          break;
        }
      }
      if (transactiondata.documents[i].data['Farmer_email'] ==
          fobj.user.email) {
        for (int l = 0; l < buyersdata.documents.length; l++) {
          if (buyersdata.documents[l].documentID ==
              transactiondata.documents[i].data['Email']) {
              ...
              child: Column(
                children: <Widget>[
                  ...
                  Padding(
                    padding: const EdgeInsets.only(top: 4.0, bottom: 10.0),
                    child: Row(children: <Widget>[
                      Text("Quantity(kg) : ${transactiondata.documents[i]
                          .data['Quantity']}",
                        style: new TextStyle(
                            fontSize: 15.0, fontWeight: FontWeight.bold),),
                      Spacer(),
                      RaisedButton(shape: RoundedRectangleBorder(
                        ...
                        print(transactiondata.documents[i].data['Buyer_Rating']);
                        if (transactiondata.documents[i].data['Buyer_Rating'] ==
                            "") {
                          dialogbox2(context, i);
                        } else {
                          dialogbox1(context, i);
                        }
                      }, child: Text("Rate Buyer", style: TextStyle(
                          fontFamily: 'Courier',
                  ...
                  RaisedButton(shape: RoundedRectangleBorder(
                    ...
                    onPressed: () async {
                      QuerySnapshot tokendata = await _firestore.collection(
                          'User Token').getDocuments();
                      for (int k = 0; k < tokendata.documents.length; k++) {
                        if (transactiondata.documents[i].data['Email'] ==
                            tokendata.documents[k].documentID) {
                      ...
                      final data = {
                        "notification": {
                          "body": "Message",
                          "title": "You got a collaboration request from ${fobj
                              .user.email}"
                        },
                        "priority": "high",
                        "data": {
                      ...
                      fobj.addcollaborationrequest({
                        'Buyeremail': transactiondata.documents[i]
                            .data['Email'],
                    ...
                    child: Text("Send collaboration request", style: TextStyle(
                        fontFamily: 'Courier',
                        fontWeight: FontWeight.bold,
                        fontSize: 15.0)),),
                  RaisedButton(
                    ...
                    onPressed: () {
                      Navigator.push(context,
                          MaterialPageRoute(builder: (context) =>
                              BidderProfile(user: transactiondata.documents[i]
                                  .data['Email'])));
                    },
                  ),
                ],
              ...
              child: Column(
                children: <Widget>[
                  ...
                  Padding(
                    padding: const EdgeInsets.only(top: 4.0, bottom: 10.0),
                    child: Row(children: <Widget>[
                      Text("Request status : ${collabdata.documents[q]
                          .data['Status']}",
                        style: new TextStyle(
                            fontSize: 15.0, fontWeight: FontWeight.bold),),
                      Spacer(),
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: Row(children: <Widget>[
                  ...
                  }, child: Text("Rate Provider", style: TextStyle(
                      fontFamily: 'Courier',
                      fontWeight: FontWeight.bold,
                      fontSize: 15.0)),),
  ...
  dialogbox2(BuildContext context,int i){
    bool isbuyerlocal=false;
    for(int k=0;k<buyersdata.documents.length;k++){
      if(buyersdata.documents[k].documentID==transactiondata.documents[i].data['Email']){
        if(buyersdata.documents[k].data['Type']=='local'){
          isbuyerlocal=true;
